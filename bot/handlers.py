...
class BotHandlers:
    ...
    async def handle_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text("🤖 扫雷系统已就绪，请在群组内使用指令。")
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if txt == "账单" and is_adm:
            ...
            pay_total = conn.execute("SELECT SUM(amount) FROM logs WHERE action LIKE '中雷收入%' AND chat_id=?", (str(cid),)).fetchone() or 0
            conn.close()
    ...
    # ... gen_amts, handle_callback, finalize 逻辑保持一致 ...
