...
class BotHandlers:
    ...
    async def handle_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if "all_logs_" in txt:
            ...
            if not logs: return await update.message.reply_text("该群暂无账单。")
        ...
        if "total_assets_" in txt:
            ...
            # --- 修正1：总账显示名字 ---
            rep = f"💰 【本群总账汇总】\n总持分合计：{sum(b for n,b in data):.2f}\n\n🏆 持分排行：\n"
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if txt == "总账" and is_adm:
            ...
            return await msg.reply_text("🔒 安全校验：请私聊查看本群总账。", reply_markup=InlineKeyboardMarkup(kb))
        ...
        if (txt.startswith('+') or txt.startswith('-')) and is_adm:
            if msg.reply_to_message:
                ...
                self.db.log_action(target.id, "上分", num, 0, cid)
    ...
    async def timer_refund(self, pid, cid, context):
        ...
        if pid in active_packets:
            ...
            if unclaimed > 0:
                self.db.add_balance(d["owner_id"], cid, unclaimed, d["owner_name"]); self.db.log_action(d["owner_id"], "过期退还", unclaimed, 0, cid)
    ...
    async def finalize(self, pid, mid, context):
        ...
        for i, g in enumerate(d["grabbers"]):
            ...
            self.db.add_balance(g['id'], d["cid"], amt, g['name']); self.db.log_action(g['id'], "抢包", amt, 0, d["cid"])
            if is_mine:
                self.db.add_balance(g['id'], d["cid"], -d["total"], g['name']); self.db.log_action(g['id'], "中雷", -d["total"], 1, d["cid"])
                inc = round(d["total"] * (1 - COMMISSION), 2)
                self.db.add_balance(d["owner_id"], d["cid"], inc, d["owner_name"]); self.db.log_action(d["owner_id"], f"中雷收入({g['name']})", inc, 0, d["cid"])
