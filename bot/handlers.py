...
class BotHandlers:
    ...
    async def handle_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if "stats_" in txt:
            ...
            for i, (t, act, amt) in enumerate(all_mines[offset:offset+20]):
                ...
                rep += f"{offset+i+1}. {t[5:16]}+{from_name}+发{amt/0.95:.0f}+抽{amt/0.95*0.05:.2f}{link_str}\n"
            btns = []
            ...
            await (update.callback_query.edit_message_text if update.callback_query else update.message.reply_text)(rep, reply_markup=InlineKeyboardMarkup([btns]) if btns else None, parse_mode="Markdown", disable_web_page_preview=True)
            return
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if p_match:
            ...
            try:
                with open("cover.jpg", "rb") as f: sent = await context.bot.send_photo(chat_id=cid, photo=f, caption=cap, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🧧 立即抢包", callback_data=f"grab_{pid}")]]))
                active_packets[pid]["mid"] = sent.message_id
            except: 
                sent = await msg.reply_text(cap, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🧧 立即抢包", callback_data=f"grab_{pid}")]]))
                active_packets[pid]["mid"] = sent.message_id
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if pid.startswith("page_"):
            p_parts = pid.split("_"); q.message.text = f"/start stats_{p_parts[1]}_{p_parts[2]}"
        ...
        if any(g['id'] == u.id for g in d["grabbers"]): return await q.answer("❌ 你已经抢过了", show_alert=True)
        ...
        if grabbed_num >= d["count"]: 
            d["status"] = "settling" # 立即标记为结算中，封锁后续点击
            if d["task"]: d["task"].cancel()
